...
> 82 agents. 17 departments. 268 skills. Multi-runtime. Framework-backed.
...
| Execution Agents | 82 agents across 17 domains (not just dev) |
...
**Gateway layer (mixed-provider routing):** Context injection alone is honour-system — the Task `model` param has no hook. To make a role that points at a *different provider* (e.g. `execution → ollama/kimi`, quality on Anthropic) take effect, launch with `ARKA_GATEWAY=1 arka-claude`. This renders a LiteLLM proxy from `models.yaml` (`core/runtime/gateway/`, `scripts/gateway-ensure.sh`) and points Claude Code at it via `ANTHROPIC_BASE_URL`, so each subagent alias physically routes to its upstream in ONE session. Constraint (Anthropic docs): `ANTHROPIC_BASE_URL` is one endpoint per process, so the 7 roles collapse onto the subagent alias slots — quality→`opus`, execution→`haiku` (hijacked to Ollama), mechanical→`sonnet`, strategy→main loop. The Anthropic key stays server-side in the proxy, never in the client env. Prereqs: `~/.arkaos/venv/bin/pip install 'litellm[proxy]'`, a local Ollama with the execution model, and `ANTHROPIC_API_KEY` in the env. Any failure degrades to a plain `claude` launch. Observability: `core/runtime/model_routing_check.py::status_summary()` reports whether the gateway is live and which route served traffic (surfaced in `/arka status`).

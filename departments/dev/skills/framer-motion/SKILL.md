---
name: dev/framer-motion
description: >
  Framer Motion for React: AnimatePresence exit animations, layout
  animations with stable keys, variant propagation and orchestration,
  gestures, and motion values that update without re-render — plus the
  decision rule for when NOT to use it.
  TRIGGER: "framer motion", "motion for React", "AnimatePresence", "layout
  animation", "anima este componente React", "exit animation", "/dev
  framer-motion".
  SKIP: the same effect achievable in pure CSS (scroll-driven, view
  transitions, entry effects) -> dev/css-native (zero dependency beats a
  library); generative canvas work -> dev/canvas-generative.
...
# Framer Motion — `/dev framer-motion`

> **Agent:** Diana (Frontend Dev) | **Framework:** Framer Motion (genjutsu)
...
parent/child coordinate, and gesture guards — plus the reason Framer
Motion was the right tier for this effect rather than CSS.

...
Apply when writing or reviewing GSAP code in Vue (or Nuxt), Svelte (or SvelteKit), or other component frameworks that use a lifecycle (mounted/unmounted). For **React** specifically, use `references/react.md` (useGSAP hook, gsap.context()).

**Related skills:** For tweens and timelines use `references/core.md` and `references/timeline.md`; for scroll-based animation use `references/scrolltrigger.md`; for React use `references/react.md`.

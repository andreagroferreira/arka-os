# Design DNA study — extracting design DNA from a screenshot or URL

> Source: near-verbatim port of `references/study.md` from the **hallmark** skill repo (MIT — see `hallmark.LICENSE` in this references directory). Adapted for ArkaOS: fetch-tool naming (WebFetch, Firecrawl), stamps rendered as `[arka:design-dna]`, state paths under `.arka/design/`. The SSRF-safety, refusal, prompt-injection, and attestation layers are kept verbatim by design — do not weaken them when editing this file.

This file is loaded when the design-DNA study protocol runs. It defines the protocol for reading a reference the user supplied — either a screenshot they attached or a URL to a live page — naming what makes it work, and producing a *diagnosis report* the user can accept or amend before any code is built.
...
URL mode trades the rhythm pass for everything else getting more accurate. If rhythm is what the user wants extracted, they should attach a screenshot instead — or alongside the URL, but the protocol still defaults to one source at a time (see the "One screenshot, one diagnosis" rule in § Limits).

### URL mode — fetch pipeline

When the input is a URL:

1. **URL refusal check.** Run the URL refuse list in § Refusal **before fetching anything**. Auto-refuse on a domain match. Marketplaces and template demos don't get a fetch call at all.
...
| HTTP status was non-2xx, or the fetch returned an error | The URL didn't resolve / blocked the request |
...
| --- | --- |
| A paid template marketplace listing (ThemeForest, Gumroad templates, Webflow templates, Framer templates, Notion templates) | Refuse. Suggest: "Tell me what you like about it and I'll build fresh from the design-system skill instead." |
...
In URL mode, run this **before** the fetch fires — don't even fetch the page. If the URL matches any pattern, refuse and offer the redirect.

| If the URL host / path is… | Then… |
| --- | --- |
| `themeforest.net/*`, `templatemonster.com/*`, `themely.com/*` (paid template marketplaces) | Refuse. *"This looks like a template marketplace listing. I won't study it. Tell me what about it you like and I'll build fresh from the design-system skill instead."* |
...
**Image mode rule.** Do not write "this is Söhne" or "this is Inter". Write "this is a neutral grotesque body" and propose 1–2 candidates from the font canon in [`typography-craft.md`](typography-craft.md) in the diagnosis.
...
Match the page to one of the twenty-one named macrostructures in the upstream hallmark catalog (`macrostructures.md` — not ported; the names travel as vocabulary). Pick the *closest*; if it's between two, name both and say which it leans toward.

For each section visible in the source, also pick an archetype from the upstream component cookbook (`component-cookbook.md`, unported — same vocabulary rule):
...
```
{
  ...
  "macrostructure":    "<name from the macrostructure catalog>",
...
After the schema is filled, map the source to one of the catalog's named themes — but **only as a candidate**. The user may pick a different theme for their build.
...
The ArkaOS port bundles four catalog themes in [`themes/`](themes/) — [carnival](themes/carnival.md), [cobalt](themes/cobalt.md), [hum](themes/hum.md), [lumen](themes/lumen.md). The other names in this table refer to the upstream hallmark catalog and travel as vocabulary only; when a bundled theme is the closest match (loud-decorative → carnival, dev-tool light → cobalt, playful multi-accent → hum, premium AI apparatus → lumen), prefer it.

If two themes are equally close, pick whichever is more *categorically distant* from any previous output for this user (read the existing CSS for a `/* [arka:design-dna] macrostructure=... */` stamp and avoid that theme's family).
...
Anti-patterns I'd skip: [list anything visible in the screenshot — bouncy
hovers, transition-all, three-feature grid, etc. If there are none, say so.]
...
**If the user says "build it":** the skill builds with the **studied DNA as the system, not a catalog theme**. Paper, accent, type roles, macrostructure, and archetypes from the diagnosis become the tokens directly. Catalog rotation is suspended for this build. The stamp records `theme: studied-DNA` with the source URL or image tag plus the actual OKLCH/font values inline:

```css
/* [arka:design-dna] macrostructure=split-studio knobs: H2 ratio=6/6, right=proof, divider=negative-space
...
1. **Fonts cannot be identified from screenshots reliably.** In image mode, the protocol names *roles* and proposes 1–2 candidates from the canon in [`typography-craft.md`](typography-craft.md) — visual font ID is wrong half the time on custom or modified faces. In **URL mode** the rule flips: the page's `@font-face`, Google Fonts `<link>`, and `next/font` declarations name the typefaces authoritatively, and the diagnosis can name them. The role still travels into the rebuilt page (the build may pick a different specific face from the canon for the user's content); the original name is recorded as a side fact.
...
Use the format defined in [`design-md-spec.md`](design-md-spec.md) § Format, with these `study`-mode adjustments:
...
2. **Add a `## Provenance` block.** Inserted between `## System` and `## Tokens`. Carries: the source mode, the URL (URL mode only) or "image (user-attached)" (image mode), the date of extraction, the attestation answer if any, and a one-line note about confidence:
   ...
   - Image mode: *"Tokens are estimated from source-image colour bands. Fonts are role-based with named candidates from the canon. Rhythm is from a vision pass on the source."*
3. **Add a `## Notes` block** at the end with the anti-patterns the diagnosis flagged as "do NOT carry over." Future runs reading the file should see these as part of the system's identity.
4. **The stamp at the top of the file** carries `studied: yes` and `DNA-source: <mode>` plus the URL or "image" tag, mirroring the `[arka:design-dna]` macrostructure stamp pattern.

### After the file is written

Same post-emission behaviour as the default build's lock-the-system flow (per [`design-md-spec.md`](design-md-spec.md) § After the file is written):

- Subsequent runs read `design.md` first; diversification inverts to consistency.
...
- If the user says *"now build me the same kind of page for my brand"*: hand off to a fresh build (the default flow) with the schema filled in as inferred design-context, and build per the standard flow — but with the studied DNA stamped.
- If the user says *"now refactor my existing site to match this DNA"*: hand off to the **redesign flow** with the schema attached. Redesign preserves the user's content; study supplied the new shape.

...
Loaded eagerly by the design-system skill whenever the catalog pick is `hum`. The palette + font stack live in `site/css/tokens.css` (upstream hallmark repo — not bundled here) under `[data-theme="hum"]`. This file carries signature moves, motion direction, voice fixtures, anti-patterns, and how Hum holds its identity against neighbouring themes. Hum is the catalog's only playful theme — its closest siblings are Coral (single-accent restraint, modern-minimal) and Bloom (warm-light atmospheric), not another playful register.

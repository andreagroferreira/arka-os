...
Loaded eagerly by the design-system skill whenever the catalog pick is `cobalt`. The OKLCH palette + font stack live in `site/css/tokens.css` (upstream hallmark repo — not bundled here) under `[data-theme="cobalt"]`. Canonical build: `site/examples/cobalt-01/` (upstream hallmark repo — not bundled here) (an API product landing with a request/response hero).

> **Why not orange.** GitBook and Firecrawl both converged on orange-as-signal — but that lane is now crowded, and the catalog's warm slots are already taken (Lumen brass, Bloom terracotta, Coral coral). Cobalt keeps their *discipline* (cool ground + one signal + code-as-hero + hairlines) and aims the signal at the open electric-blue lane instead. The blue is the differentiator, not a copy.
...
Plus the Space Grotesk + Inter + JetBrains Mono link and the small reveal/type-in script. The canonical build to mirror is `site/examples/cobalt-01/` (upstream hallmark repo — not bundled here).

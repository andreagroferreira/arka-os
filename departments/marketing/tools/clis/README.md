...
> **Security note:** `zapier.js hooks send` is the one CLI whose destination
> host is caller-controlled — it POSTs arbitrary `--data` to any HTTPS URL
> you pass. Review the URL before invoking it from an agent context.
> `wistia.js captions create --srt-file` only accepts `.srt`/`.vtt` caption
> files that resolve (symlinks followed) to inside the current working
> directory — both the argument and the final symlink target must be a
> caption file, and hard links are rejected — so no in-tree link, soft or
> hard, can redirect the read to a secret.

---
name: mkt/competitor-profile
description: >
  Research and profile competitors from their URLs into structured,
  comparable dossiers — scrape positioning, features, pricing, and social
  proof, layer in SEO and review data, and persist raw data for auditing.
  TRIGGER: "competitor profile", "competitor research", "competitor
  analysis", "profile this competitor", "analyze competitor", "competitive
  intelligence", "competitor deep dive", "who are my competitors",
  "competitor landscape", "competitor dossier", "competitive audit",
  "research these competitors", "perfil de concorrente",
  "/mkt competitor-profile". Input is a list of competitor URLs; output is
  one structured profile file per competitor. SKIP: building the comparison
  / "X vs Y" / alternatives page from the profiles -> mkt/competitor-analysis;
  the ongoing competitive-intelligence brief as a KB artifact ->
  kb/competitive-intel.
...
# Competitor Profiling — `/mkt competitor-profile`

> **Agent:** Lourenço (Product Marketing Manager) | **Frameworks:** Structured competitive profiling, Positioning maps, SEO + review triangulation
...
The project-local `competitor-profiles/` tree stays in the repo for raw
data and auditing. File the synthesized `<competitor>.md` profiles and the
`_summary.md` to Obsidian under `WizardingCode/Marketing/Competitors/` as
the KB-first source of truth, so positioning, sales, and content work read
from the same dossiers.

## Related ArkaOS skills

- **`mkt/competitor-analysis`** — build the comparison / alternative page from these profiles.
- **`kb/competitive-intel`** — file the ongoing competitive-intelligence brief in the KB.
- **`mkt/customer-research`** — mine reviews and community sentiment in depth.
- **`mkt/content-strategy`** — use competitor content gaps to plan your own content.
- **`mkt/seo-audit`** — audit your own site relative to these competitors.
- **`mkt/paid-campaign`** — analyze competitor ad strategies.
- **`saas/pricing-strategy`** — deeper pricing analysis informed by the profiles.
- **`mkt/audience-segment`** — broader list-building qualification from the landscape.

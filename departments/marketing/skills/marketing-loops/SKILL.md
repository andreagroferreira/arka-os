...
description: >
  Set up a recurring, self-running marketing workflow — a repeatable loop
  an AI agent runs on a cadence (weekly, daily, on a trigger) rather than a
  one-off task. Escolhe, adapta e agenda um loop de marketing contínuo que
  orquestra as outras skills de marketing. TRIGGER: "marketing loop",
  "recurring marketing workflow", "automate my marketing", "marketing on
  autopilot", "weekly marketing review", "ad fatigue check", "content
  refresh loop", "churn watch", "ranking drop alert", "always-on
  marketing", "marketing automation workflow", "run this every week", "/mkt
  marketing-loops". SKIP: one-off marketing ideas -> mkt/marketing-ideas
  (this schedules the repeatable ones, that inspires); the experimentation
  loop specifically -> mkt/ab-test.
...
# Marketing Loops — `/mkt marketing-loops`

> **Agent:** Luna (Marketing Director) | **Frameworks:** Growth Loops, AARRR, PDCA (Deming Cycle)
...
- **`mkt/marketing-ideas`** — one-off tactics and inspiration (what to try). Loops operationalize the ones worth repeating.
- **`mkt/ab-test`** — the experimentation loop specifically (hypothesis → test → promote winner → repeat).
- **`mkt/analytics-report`** — most loops read from analytics to decide whether to act.
- **`ops/workflow-automate`** — the automation and scheduling mechanics a loop runs on.
- **Channel skills the loop bodies orchestrate** — `mkt/paid-campaign`, `mkt/seo-audit`, `mkt/email-sequence`, `mkt/social-strategy`, `saas/churn-analysis`, `saas/pricing-strategy`, `landing/affiliate-bridge`.
- **`/arka research`** — for research-style loops (competitor changes, social listening) that need a fresh external scan.

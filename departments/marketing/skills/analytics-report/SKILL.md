...
Produces the marketing analytics report — the AARRR pirate-metrics funnel,
channel performance breakdown, and CAC trends — and sets up the tracking
that feeds it. A report is only as honest as its instrumentation, so this
skill covers both the measurement plan (events, GA4, GTM, UTMs) and the
reporting layer on top. The goal is tracking that provides actionable
...
**For the full event library**: See [references/event-library.md](references/event-library.md)
...
**For the full event library by business type**: See [references/event-library.md](references/event-library.md)
...
- **`landing/ab-test`** — experiment tracking and measurement
- **`mkt/seo-audit`** — organic traffic analysis
- **`landing/optimize-page`** — conversion optimization (uses this data)
- **`sales/revops`** — pipeline metrics, CRM tracking, and revenue attribution

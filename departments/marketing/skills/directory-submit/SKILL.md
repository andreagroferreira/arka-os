---
name: mkt/directory-submit
description: >
  Plan the directory layer of a product launch or an ongoing backlink
  campaign — submit to startup / SaaS / AI / agent / MCP / no-code / review
  directories for dofollow backlinks, domain rating, discovery, and AI
  citations, wired to destination pages that actually convert. Submissão a
  diretórios e campanha de backlinks (Product Hunt, G2, TAAFT...). TRIGGER:
  "directory submissions", "submit to directories", "backlinks from
  directories", "list my product", "submit to Product Hunt", "BetaList",
  "TAAFT", "Futurepedia", "G2 listing", "Capterra listing", "AlternativeTo",
  "SaaSHub", "AI directories", "MCP registry", "agent directory", "dofollow
  backlinks", "launch directories", "directory tracker", "/mkt
  directory-submit". SKIP: the broader launch moment -> saas/launch-execute;
  the programmatic destination pages backlinks flow into ->
  mkt/programmatic-seo; AI citation optimization -> mkt/ai-seo.
...
# Directory Submissions — `/mkt directory-submit`

> **Agent:** Ana (SEO Specialist) | **Frameworks:** Foundation-Before-Submission, Product Hunt Anchor, 10-in-30 Reviews, GEO
...
Use the product marketing context (see Context above) and only ask for
...
- **`saas/launch-execute`** — the broader launch moment and five-phase approach.
- **`mkt/programmatic-seo`** — destination pages (alternatives, integrations, templates) backlinks should flow into.
- **`mkt/competitor-analysis`** — the `/alternatives/[tool]` page pattern.
- **`mkt/ai-seo`** — GEO optimization for AI citation.
- **`mkt/content-strategy`** — editorial content that attracts "best of" listicle inclusions.
- **`mkt/schema-markup`** — FAQ + Product + Organization JSON-LD for GEO.

...
name: mkt/council
description: >
  Simulated board of 12 legendary marketers (Godin, Ogilvy, Schwartz,
  Hopkins, Halbert, Brunson, Hormozi, Dunford, Sutherland, Sharp,
  Handley, Vaynerchuk) — each gives their take through their documented
  frameworks, the council surfaces where they DISAGREE, and a chair
  synthesizes a recommendation. TRIGGER: "marketing council", "conselho
  de marketing", "board of advisors", "what would Seth Godin say", "what
  would Ogilvy think", "channel Hormozi", "get multiple perspectives on
  this", "debate this", "have the council review", "/mkt council". SKIP:
  the personal/strategic advisory board (non-marketing) -> /arka conclave;
  executing the winning direction -> the execution skill
  (mkt/product-marketing, landing/offer-create, landing/copy-framework,
  mkt/paid-campaign).
...
# Marketing Council — `/mkt council`

> **Agent:** Luna (Marketing Director) | **Frameworks:** 12-advisor bench, disagreement mapping, chair synthesis
...
**This is persona simulation, not the real people.** Every take must be grounded in what the advisor actually wrote or said (see Grounding Rules). Label the output as simulation. This is the marketing-specialist board; for a personal/strategic advisory board across all domains, use `/arka conclave`.
...
When the topic is specific (a niche, a channel shift, a current platform change) or the user wants sources, go beyond the dossiers — KB-first, then external:

- **KB-first:** `mcp__obsidian__search_notes` for existing notes on the advisor or topic, cited as `[[wikilinks]]`.
- **Fan-out research** (`/arka research`): find what the seated advisors have actually said or written about this topic class — books, essays, interviews, podcasts — plus current state of the debate.
- **Otherwise**: use web search (Firecrawl/WebSearch) for `[advisor name] + [topic]` per seated advisor, preferring primary sources (their own books, blogs, newsletters, talks) over roundup articles.
...
- **`mkt/product-marketing`**: When Dunford's take wins — execute the positioning work
- **`landing/offer-create`** / **`saas/pricing-strategy`**: When Hormozi/Halbert direction wins — build the offer
- **`landing/copy-framework`**: When the council reviewed copy — execute revisions
- **`mkt/paid-campaign`**: When the debate was media or creative strategy
- **`mkt/social-strategy`**: When Handley/Vaynerchuk direction wins
- **`mkt/ab-test`**: When the disagreement map says "test it" — Hopkins would insist
- **`/arka research`**: For the live research pass
- **`/arka conclave`**: The personal/strategic advisory board across all business domains (this council is marketing-only)

...
This file is loaded after the macrostructure pick, when you reach the "Decide on hero enrichment" step. It tells you whether to enrich the hero with media at all, and if so, which archetype and how to build it.
...
| SaaS landing, manifesto, agency, studio, atmospheric, slow-and-editorial | **Kit-led.** Hosted abstract-asset kits are not bundled — generate via the ArkaOS image pipeline (`/brand image-create`, ComfyUI) or source per the register's imagery rules. |
...
- When the user has attached an image asset (or `.arka/design/preflight.json` cached one), use it. Never overwrite with a placeholder.
...
- Imagery rows above don't override genre overlays. A modern-minimal register still suppresses decorative kit imagery.

The hierarchy below picks the tier *after* this gate decides imagery is needed at all. Skipping this gate is what produces "blob illustration on every page" outputs — exactly the AI-default this doctrine refuses.
...
| **C · Generated illustration** | Generated via the ArkaOS image pipeline (`/brand image-create`, ComfyUI), with provenance + post-processing. | Characters or specific scenes that hand-build can't economically reach. Always post-processed. |
...
Tiers A and B are hand-built (pure CSS art, hand-drawn SVG — the upstream `custom-craft.md` canon). Tiers C, D, and E source via the ArkaOS image pipeline (`/brand image-create`, ComfyUI) or the vetted libraries above.
...
If the brief contains explicit visual cues, pick from this map:
  ...
  • "shop", "store", "menu", "products", "items"          → E8 photography (or product grid)
  ...
  • Austere / quiet register picked                       → no enrichment (the register IS restraint)
...
**Critical:** never `loading="lazy"` on the hero video — that kills LCP. Use `preload="metadata"` and `fetchpriority="high"`. Always include a `poster=""` and a `<track kind="captions">` for accessibility. Any deliberately overflowing element needs the global `overflow-x: clip` safety net — see [`layout-craft.md`](layout-craft.md) § Container & grid.
...
*Avoid when:* the register already has a paper feel (specimen-, atelier-, or risograph-style registers). Doubling the grain is muddy.
...
Existing photography archetype in the cookbook — cross-referenced here for completeness. See [`component-cookbook.md`](component-cookbook.md) for variation knobs.

**Example.** A small Lisbon café. One tightly-cropped photograph of the espresso machine at dawn, 4/3 ratio, no full-bleed. Caption sits margin-aligned at lower-left in mono small-caps ("Plate 04 · 6:42 a.m."). The photograph is desaturated 8 % from the source to harmonise with the page's warm-paper tone. Always pair photography with a tone-matched typography pairing — a luxury-tone photo on a brutalist page jars.
...
<header class="hero hero--overflow">
  ...
  <p class="hero__lede">ArkaOS. A design doctrine that refuses defaults.</p>
</header>
```
```css
...
The hero is the highest-stakes animation surface on the page; the rules are tighter here than elsewhere.
...
**Reduced-motion is the default in 2026.** Every animation gets a `@media (prefers-reduced-motion: reduce)` block that either disables the motion or replaces it with a static keyframe. This is non-negotiable; the review gate will catch you.
...
8. Does its tone match the page's tone? (Risograph illustration on a brutalist page = wrong. Hand-drawn doodle on a developer-tool page = wrong. Three.js bloom on a quiet, austere page = wrong.)

The design-review skill (`departments/brand/skills/design-review`) carries binary gates that mirror these questions; run it before the page ships.
...
```css
/* [arka:design-dna] macrostructure=marquee-hero knobs: H1 size=xxl, alignment=left-bias
...
This signals to future runs (and to design-review) what was chosen and how. It also lets the user see the inferences in one place and redirect if anything's off.
...
- **Adding a grain background everywhere.** Grain is a treatment, not a default. Registers with print-craft DNA already carry texture. Don't double up.

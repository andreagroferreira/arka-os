...
SCRIPTS_DIR = (
    ...
    / "departments" / "dev" / "skills" / "watch" / "scripts"
)
...
def _isolated_home(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    """No watch test may read or write the real user home (Wave 3 rule).

    Config, keys, and telemetry all resolve at call time under HOME /
    ARKAOS_HOME, so redirecting both here isolates every in-process call
    AND every subprocess run that inherits os.environ.
    """
    home = tmp_path / "home"
    home.mkdir()

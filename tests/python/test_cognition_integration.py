...
def make_capture(**overrides) -> RawCapture:
    defaults = {
        ...
        "project_path": "/Users/andre/Herd/rockport",
...
class TestCognitionIntegration:
    ...
    def test_full_cycle(self, tmp_path: Path) -> None:
        ...
        pending = insight_store.get_pending("rockport")
    ...
    def test_dual_write_search_cross_project(self, tmp_path: Path) -> None:
        ...
        # a. Write a KnowledgeEntry from project "rockport" applicable to laravel
        entry_rockport = make_knowledge_entry(
            title="Rockport: Sanctum Rate Limiting",
            source_project="rockport",
            applicable_to="laravel",
            tags=["laravel", "sanctum", "rate-limiting"],
            content="Apply throttle middleware to Sanctum API routes to prevent abuse.",
        )
        result_rockport = dual_writer.write(entry_rockport)
        assert result_rockport.vector_indexed is True

        # b. Write a KnowledgeEntry from project "fovory" applicable to laravel
        entry_fovory = make_knowledge_entry(
            title="Fovory: Sanctum Token Scopes",
        ...
        result_fovory = dual_writer.write(entry_fovory)
        assert result_fovory.vector_indexed is True
        ...
        assert entry_rockport.title in found_titles
        assert entry_fovory.title in found_titles

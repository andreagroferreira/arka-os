...
class TestBuildReport:
    ...
    def test_report_with_results(self) -> None:
        ...
        skill_results = [_skill("rockport", "updated")]
    ...
    def test_report_collects_errors_from_all_phases(self) -> None:
        ...
        skill_results = [_skill("fovory", "error", error="missing file")]
...
class TestFormatReport:
    ...
    def test_format_shows_skill_feature_additions(self) -> None:
        skill_results = [
            _skill("fovory", "updated", features_added=["forge"]),
            _skill("rockport", "updated", features_added=["forge"]),
        ]
        report = build_report("v2.13.0", "v2.14.0", [], [], [], skill_results)
        output = format_report(report)

        assert "forge" in output
        assert "fovory" in output
    ...
    def test_format_skills_line_label(self) -> None:
        skill_results = [_skill("rockport", "updated"), _skill("fovory", "unchanged")]
        report = build_report("v2.13.0", "v2.14.0", [], [], [], skill_results)
        output = format_report(report)

// Tests for the ~/.arkaos/config.json seed/migration logic (PR19 v2.41.0).
//
// Contract: seedArkaosConfig is idempotent. Writes hardEnforcement=true
// only when the key is unset (file absent or hooks.hardEnforcement undefined).
// Explicit user choice (true OR false) is preserved unchanged.
...
test("seed is idempotent on hardEnforcement=true", () => {
  const { dir, cleanup } = makeTmpHome();
  try {
    ...
    // No clobber, no whitespace churn when value already correct.

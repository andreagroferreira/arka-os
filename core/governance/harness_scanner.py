...
# 2.1.257 floor; the gate sets and the manifest dropped it in Runtime Sync
# PR1), but it is NOT retired from the permission grammar: the 2.1.259
# binary still validates a `MultiEdit(...)` rule by mapping it onto `Edit`
# (rule validation in the permission engine, proven by inspecting the
# binary). An unscoped `MultiEdit` allow therefore still grants every
# Edit, so the name stays in this pattern and in _SCOPE_HINT.

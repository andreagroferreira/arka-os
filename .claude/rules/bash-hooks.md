...
- Never block on unbounded network calls: any outbound call from a hook is bounded (≤ 1.5 s hard timeout, ≤ 0.3 s inside the Synapse bridge), cached, skipped once the budget deadline passes, and fails open — precedent: Graphify L2.5, Jev decisions stage (UserPromptSubmit; one call, plus at most one skill-hints repair call inside the same budget), Jev bash-effect (PreToolUse, Python path only, 1000 ms, never for an effect or empty command) and Jev Stop sites (Stop; one call for the four sites, 1200 ms inside the 3 s `ARKA_STOP_BUDGET_MS` budget, never the transcript)

...
| `/arka decisions [period]` | Jev typed-decision telemetry (`~/.arkaos/telemetry/decisions.jsonl`) by day/week/month/all: per site: calls, agreement with the heuristic, fallback rate, share acted on Jev, p50 latency and cost; overall: total cost, p50 latency, cache-hit rate and the top fallback reasons. Shells out to `~/.arkaos/bin/arka-py -m core.decisions.telemetry_cli <period>`. |

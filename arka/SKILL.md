...
| `/arka decisions [period] [--by-site]` | Jev typed-decision telemetry (`~/.arkaos/telemetry/decisions.jsonl`) by day/week/month/all: per site: calls, agreement with the heuristic, fallback rate, share acted on Jev, p50 latency and cost; overall: total cost, p50 latency, cache-hit rate and the top fallback reasons. Always ends with **Cost and effect**: measured Jev cost from the LLM ledger (production `decision` apart from replay `decision-replay`), then one row per cost site (#7 forge-complexity, #10 dispatch-role, #11 qg-prescreen, #22 subagent-discipline) with its effect count and a saving column that reads `—` with the reason, or `counterfactual (ceiling)` for the qg-prescreen reviewer cost. No measured saving is claimable today. `--by-site` adds a per-site table: measured cost, acted on Jev, fallback, abstain, abstain attributable to Jev (abstain over answered plus abstain; egress, timeout, backoff and HTTP are not Jev's) and the top fallback reasons. Shells out to `~/.arkaos/bin/arka-py -m core.decisions.telemetry_cli <period> [--by-site]`. The replay evidence report is a separate command: `~/.arkaos/bin/arka-py -m core.decisions.replay_report (--all or --sites a,b) --session <id> [--runs 2]`. |

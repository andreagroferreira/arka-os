...
class TestRawCapture:
    def test_create_valid_capture(self):
        ...
        capture = RawCapture(
            session_id="sess-001",
            project_path="/Users/dev/projects/fovory",
            project_name="fovory",
            category="decision",
...
class TestCaptureStore:
    ...
    def test_get_by_project(self, store, sample_capture):
        store.save(sample_capture)
        results = store.get_by_project("fovory")
        assert len(results) == 1
...
class TestInsightStore:
    ...
    def test_save_and_retrieve(self, store, sample_insight):
        ...
        results = store.get_pending("fovory")
        assert len(results) == 1
    ...
    def test_get_all_pending(self, store):
        ...
        for project in ["fovory", "rockport", "clubefashion"]:
            store.save(
                ActionableInsight(
                    project=project,
                    trigger="dreaming",
    ...
    def test_dismissed_count_by_category(self, store):
        ...
        counts = store.dismissed_counts("fovory")
    ...
    def ecosystems_path(self, tmp_path):
        ...
        eco_file.write_text(json.dumps({
            "ecosystems": {
                "fovory": {
                    "projects": [
                        {"name": "fovory-supplier-sync", "stack": "laravel", "path": "/Users/dev/Herd/fovory"},
                        {"name": "fovory-shopify-theme", "stack": "shopify-liquid", "path": "/Users/dev/Work/fovory-theme"},
                    ],
                    "domain": "ecommerce",
                },
                "zugatv": {
                    "projects": [
                        {"name": "zugatv-frontend", "stack": "nuxt", "path": "/Users/dev/Work/zugatv"},
                    ],
                    "domain": "media",
                },
            }
...
class TestCognitionIntegration:
    ...
    def test_full_cycle(self, env):
        ...
        capture = RawCapture(
            ...
            project_path="/Users/dev/Herd/fovory",
            project_name="fovory",
            category="decision",
            content="Used Repository pattern for product sync in Fovory",
        ...
        entry = KnowledgeEntry(
            ...
            stacks=["laravel", "php"],
            content="Use Repository pattern to abstract Eloquent queries for product synchronization. Separates business logic from data access. Validated across Fovory and Rockport projects.",
            source_project="fovory",
            applicable_to="laravel",
        )
        ...
        insight = ActionableInsight(
            ...
            context="Observed during Fovory product sync implementation",
        )
        env["insight_store"].save(insight)

        # 7. Verify insight is pending for fovory
        pending = env["insight_store"].get_pending("fovory")

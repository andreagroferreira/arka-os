...
| Design detector | external npm CLI `impeccable` (pinned `^3.2` by the installer), shelled by the QG check `design-slop` in `core/governance/evidence_checks.py::_check_design_slop` | Dependency only — no upstream code vendored; the gate never installs (`npx --no-install`) |
...
Material derived from this project — three of its fifteen skills:
...
Deliberately not carried over — the remaining twelve skills, with the
